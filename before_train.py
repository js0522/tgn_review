...
#cpu    
  ...
  if not INFERENCE_ONLY: 
    ...
    num_batch = math.ceil(num_instance / BATCH_SIZE) # 200 per batch/ 406 batches

    logger.info('train split: {}'.format(TRAIN_SPLIT))
    logger.info('num of training instances: {}'.format(num_instance))
    
    logger.info('num of batches per epoch: {}'.format(num_batch))
    idx_list = np.arange(num_instance)   # js) 0 to 81029-1
    ...
    early_stopper = EarlyStopMonitor(max_round=args.patience)  # 5 in a row no improvment -- stop